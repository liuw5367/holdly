...
**操作按钮**：资产与订阅详情底部操作统一使用 secondary 浅色背景，删除使用 destructive。按钮高 40px、文字 13px、图标 14px；取消订阅使用停止循环图标。价值历史和续费历史记录的删除入口使用低对比度小叉号，位于右下角、日期下方，保留删除确认。底部不同操作使用不同语义图标：估值为趋势图、维修为工具、保修为盾牌、卖出为钱币。价值趋势图左侧展示金额刻度。
